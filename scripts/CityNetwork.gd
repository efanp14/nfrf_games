...
## base_stress per link is NOT taken from the dataset's link capacity, which is
## what it used to be and what this note used to say. Since 10 Aug 2026 it comes
## from each link's role in the network: the 24 links the simulated residents
## ride most are arterials near 0.82, the other 45 are backstreets near 0.22.
## The full derivation, and why the old capacity-based column had to go, sits
## above the `edges` array in _build_network.
...
class Link:
	...
	func effective_beta(alpha: float) -> float:
		return CityNetwork.beta_for(upgrade_level, stress_score, alpha)

	func impedance(alpha: float) -> float:
		return effective_time() * (1.0 + alpha * effective_beta(alpha) * stress_score)


## Infrastructure relief for a given level, road stress and rider, without
## needing a Link in hand.
##
## Split out of Link.effective_beta() on 30 Aug 2026 because the MAP needed it
## and had been guessing instead: LinkSegment carried its own
## `beta_est = [1.0, 0.65, 0.3]`, which no rider ever actually experiences, and
## drove the stress-view colour, the number of cars and their speed off it.
##
## The error was not small and it changed sign with personality. A cautious rider
## protecting an arterial really reaches 0.08 effective stress -- green, a large
## visible win -- while the map drew 0.25 and looked mediocre. A confident rider
## protecting the same road really reaches 0.49, barely better than the 0.53 they
## started painted at, while the map drew the same 0.25 and looked like a big
## improvement. So the one cue that answers "did what I just bought do anything"
## understated the answer for some riders and overstated it for others, which is
## how a playtester came to report improvements appearing on roads they had not
## touched and missing from roads they had.
##
...
##
## Destinations rewritten 9 Aug 2026: drawn from the existing hub pool at
## 18-35 min out, replacing the previous nearest-node rule that left a third
## of the city with no route choice. The per-neighbourhood breakdown is inline
## below, with each destination's distance from its home node.
...
# =============================================================================
#  THE BOARD ITSELF — 46 nodes, 69 undirected links
#
#  The BASE topology is the "Sioux Falls" transportation-research test network
#  (24 nodes, 38 undirected links), which is where the node numbering and the
#  published-diagram layout come from. It is not what ships: the West Extension
#  added nodes 27-47 on 4 Aug 2026. Count them from the arrays below rather
#  than from any prose, this line included.
#
#  Node IDs are the dataset's own numbering. Vector2i(n, 0) — the 0 is unused,
#  it just satisfies Vector2i's two-int shape so the rest of the codebase's
#  Vector2i-keyed adjacency and link-id machinery needs no special case.
#
#  Positions are laid out on the network's standard published diagram
#  (bstabler/TransportationNetworks), NOT the dataset's raw lat/long, which
#  clusters several nodes (16/17/19) about 20px apart at any sane canvas scale
#  and produces a tangle instead of the clean layout the diagram shows. No node
#  carries a participant-facing name.
#
#  Both tables were declared inside _build_network(), which made that function
#  248 lines of which about ten were logic. They are pure data and never
#  change at runtime, so they are constants here and the function that reads
#  them is short enough to see.
# =============================================================================
...
## Registers the board: every node, every link, the player's home/work pair for
## this session, and the handful of links that start already upgraded.
##
## Short because NODES and EDGES are constants above rather than literals in
## here. `pair_index` selects which of HOME_WORK_PAIRS this session plays.
